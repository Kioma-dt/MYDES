...
u64 ReverseLastRound(u64 ciphertext, u8 key, u8 table) {
    ...
    u32 R15 = L16;
    u32 L15 = R16 ^ DesFunc(L16, ((u64)key << (46 - table * 6)));

    return ((u64)R15 << 32) | L15;
}

u8* GetKey_8(u64* cookeys, u8 table){
    u8 *cookeys_8 = new u8[16];
    for (int i = 0; i < 16;i++){
        cookeys_8[i] = (cookeys[i] >> (46 - table * 6)) & 0xff;
    }
    return cookeys_8;
}

std::bitset<8> GetChipher_8(u64 ciphertext, const u8 bits[4]){
    u32 left = (ciphertext >> 32) & 0xffffffff;
    u32 right = ciphertext & 0xffffffff;
    return std::bitset<8>((right >> bits[0]) & 0b1 | (right >> bits[1]) & 0b10 | (right >> bits[2]) & 0b100 | (right >> bits[3]) & 0b1000 |
                        (((left >> bits[0]) & 0b1 | (left >> bits[1]) & 0b10 | (left >> bits[2]) & 0b100 | (left >> bits[3]) & 0b1000)<< 4));
}
